-- =====================================================
-- An open session means children can be checked in
-- =====================================================
--
-- check_in_children enforced the kids_events check-in window in addition to
-- the session's own status, so check-in only worked between 45 minutes before
-- a service and 30 minutes after it ended.
--
-- Reported from the desk as "nothing happens on check in click": the session
-- was open, the family was found, the children were selected, and the RPC
-- raised `outside_check_in_window` — an identifier the UI had no message for.
--
-- The gate is redundant and wrong. `status = 'open'` already answers the
-- question: a session is open because the scheduler opened it or because a
-- leader did, and either way somebody has decided children may be checked in.
-- The window belongs to auto-open and auto-close, which is where it stays. A
-- session left open too long is auto-closed; that is not a reason to refuse a
