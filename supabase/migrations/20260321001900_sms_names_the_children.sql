-- =====================================================
-- The pickup text should name the children
-- =====================================================
--
-- 20260321001800 queued the text immediately after the secret was minted,
-- which is before check_in_one_child has inserted a single row. So the message
-- went out as
--
--     "ALIC Kids pickup code: 5HRA. Keep this — you will need it to collect."
--
-- with the "for Kidus, Liya" clause silently empty, because the batch had no
-- children on it yet. The code was right and the text was still useful, which
-- is exactly why this would have survived a casual look.
--
-- Moved to after the loop. It remains the only moment the code can be sent —
-- it is hashed with a Vault pepper on the way into kids_check_in_secrets and
