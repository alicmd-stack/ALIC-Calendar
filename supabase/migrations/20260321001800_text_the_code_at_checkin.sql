-- =====================================================
-- Text the pickup code at check-in
-- =====================================================
--
-- The printed slip is currently the only copy of the code, and there was no
-- reprint until 20260321001700 — so a slip left in the car meant an override,
-- which is meant to be the exceptional two-person act and not the routine
-- remedy for mislaid paper.
--
-- Queued from inside check_in_children because that is the ONLY place the
-- plaintext code exists. It is hashed with a Vault pepper on the way into
-- kids_check_in_secrets and never stored, by design, so there is no later point
-- at which anything could look it up and send it.
--
-- This queues real rows today. Delivery waits on Twilio credentials — the edge
-- function already fails a non-email row cleanly with "no provider configured",
-- so nothing breaks in the meantime and nothing further changes here when the
-- secrets are added.
...
BEGIN
  ...
  -- Text the code while it is still in scope. It is a peppered HMAC once
  -- stored, so this is the only moment it can ever be sent. Never raises: a
  -- failed text must not roll back a check-in that has already happened.
